...
public class TileExtenderController {
  private final ImageExtenderService imageExtenderService;
    ...
    @PutMapping("/extend")
    public ResponseEntity<String> extendImage(@RequestBody TileExtenderRequestBody body) throws Exception {
        int x = body.getX();
        int y = body.getY();
        int z = body.getZ();
        ...
        String base64Encoded = imageExtenderService.process(x, y, z, server, layer, shiftNb, isCropped, lat, lon);
